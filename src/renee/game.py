...
class Game:
    ...
    - ActionPipeline (action processing with event appliers)
    ...
    def __init__(
        ...
        # ActionPipeline integrates with World and has event appliers
        ...
        for schema_file in self.config.schema_files:
            try:
                self.schema_registry.load_from_path(schema_file)
    ...
    def init_turn_manager(self, turn_manager: TurnManager) -> None:
        ...
        # Update action pipeline with turn manager
        self.action_pipeline.turns = turn_manager
        ...
        Example:
            ...
            def render_entities():
                ...
                for entity in game.world.query(Position, Sprite):
                    ...
                    commands.append(DrawSprite(sprite.name, pos.x, pos.y))
                return commands
        """
    ...
    def _should_quit(self) -> bool:
        """Check if the game should quit based on input state."""
        if not self.renderer:
            return False
        try:
            input_state = self.renderer.get_input()
            return getattr(input_state, "quit_requested", False)
        except Exception:
            return False

    def _process_input(self) -> None:
        """Process input from renderer."""
        # Input is retrieved via renderer.get_input()
        # Game-specific input handling should be done in on_update hooks
        pass
    ...
    def _render(self) -> None:
        ...
        commands.sort(key=lambda cmd: getattr(cmd, "layer", 0))
    ...
    def get_input(self) -> "InputState | None":
        """Get current input state from renderer.

        Returns:
            InputState if renderer is available, None otherwise.
        """
        if not self.renderer:
            return None
    ...
    def export_state(self) -> dict[str, Any]:
        ...
        state: dict[str, Any] = {
            ...
            "event_history": [
                {"type": e.type, "payload": e.payload} for e in self.event_bus.history()
            ],
        }

        if self.turn_manager:
            state["turn_state"] = self.turn_manager.to_dict()
    ...
    def export_state_json(self, filepath: str | Path | None = None, indent: int = 2) -> str:
        ...
        json_str = json.dumps(state, indent=indent, default=str)
