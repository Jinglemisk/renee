...
class ImpactAnalyzer:
    ...
    def find_dependencies(self, component: str) -> list[str]:
        ...
        # ADAPTED: use rules() instead of list_rules()
    ...
    def check_intents(self, change: dict[str, Any]) -> list[str]:
        ...
        # ADAPTED: use try/except since has() doesn't exist, and get() needs kind parameter
        schema = None
        try:
            schema = self.schema_registry.get(target, kind="component")
        except Exception:
            # Schema not found or wrong kind - try other kinds
            for kind in ["event", "action"]:
                try:
                    schema = self.schema_registry.get(target, kind=kind)
                    break
                except Exception:
                    pass

        if schema and hasattr(schema, 'intent') and schema.intent:
